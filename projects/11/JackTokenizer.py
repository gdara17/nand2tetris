...
class JackTokenizer:
    ...
    KEYWORD_PATTERN = (
        r"(?P<keyword>\s(class|constructor|function|method"
        r"|field|static|var|int|char|boolean"
        r"|void|true|false|null|this|let|do"
        r"|if|else|while|return)\s)"
    )
    ...
    def __token_generator(self):
        for match in self.TOKEN_RE.finditer(self.source):
            yield next(
                (group, token.strip().strip('"')) for group, token in match.groupdict().items() if token
            )
...
if __name__ == "__main__":
    ...
    outfilenames = [
        infilename.replace(".jack", "T.xml") for infilename in infilenames
    ]
